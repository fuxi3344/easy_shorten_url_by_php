...
function RunSQL($sql, $back, $bind)//MySQL语句执行函数,参数：SQL语句, 是否需要返回, 绑定参数数组
{
	...
	if ($back)
	{
		if (count($bind) > 0)
		{
			foreach($bind as $key => $value)//转换数组类型
			{
				$refs[] = &$bind[$key];
			}
			call_user_func_array(array($stmt,'bind_param'), $refs);//数组为参数调用对象的函数
			if ($stmt->execute())
			{
				//绑定参数 返回 执行成功
